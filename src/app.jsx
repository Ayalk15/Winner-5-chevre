...
export default function App() {
  ...
  const loginAsAdmin = () => {
    const pass = isAdminMode ? 'out' : prompt('סיסמת מנהל:');
    if (pass === '2531') setIsAdminMode(true);
    else if (pass === 'out') setIsAdminMode(false);
  ...
  const getLiveStatistics = () => {
    ...
    Object.keys(actualScores).forEach(key => {
      ...
      if (actual && actual.isFinished && pred) {
        ...
        if (jokers[md] && String(jokers[md]) === String(gameId)) p *= 2;
        matchPoints += p;
      }
    });
  ...
  const currentMatchdayScore = () => {
      let p = 0;
      allFixtures[matchday]?.forEach(g => {
        const key = `${matchday}-${g.id}`;
        const actual = actualScores[key];
        const pred = predictions[key];
        if (actual && actual.isFinished && pred) {
           if (pred.winner === actual.winner) { p += 2; if (Number(pred.homeScore) === Number(actual.homeScore) && Number(pred.awayScore) === Number(actual.awayScore)) p += 4; }
           if (jokers[matchday] && String(jokers[matchday]) === String(g.id)) p *= 2;
        }
      });
      return p;
  };
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4 pb-24" style={{ direction: 'rtl' }}>
      <div className="sticky top-0 bg-gray-950/95 backdrop-blur-md pt-2 pb-3 z-50 max-w-md mx-auto border-b border-gray-900/50">
        <header className="text-center">
          <h1 className="text-xl font-extrabold text-yellow-500">🏆 10 חבר'ה - יוספטל</h1>
          <div className="text-[10px] text-gray-400 font-bold mt-1">{liveClockText}</div>
        </header>
        <nav className="grid grid-cols-5 gap-0.5 bg-gray-900 p-1 rounded-xl mt-2">
          {['predictions', 'tournament', 'stats', 'leaderboard', 'rules'].map(tab => (
            <button key={tab} onClick={() => setCurrentTab(tab)} className={`py-1.5 text-[9px] font-black rounded-lg ${currentTab === tab ? 'bg-yellow-500 text-gray-950' : 'text-gray-400'}`}>
              {tab === 'predictions' ? '⚽ משחקים' : tab === 'tournament' ? '👑 הטורניר' : tab === 'stats' ? '📈 סטט'' : tab === 'leaderboard' ? '📊 טבלה' : 'ℹ️ חוקים'}
            </button>
          ))}
        </nav>
      </div>
      ...
      <div className="max-w-md mx-auto mt-4 space-y-4">
        {currentTab === 'predictions' && (
          <>
            <div className="bg-gray-900 p-3 rounded-xl space-y-2">
              <select value={matchday} onChange={(e) => setMatchday(Number(e.target.value))} className="w-full bg-gray-800 p-2 rounded-lg font-bold">
                {[...Array(36).keys()].map(i => <option key={i+1} value={i+1}>מחזור {i+1}</option>)}
              </select>
              <div className="text-center text-xs font-bold text-amber-500">{countdownText}</div>
            </div>
            {allFixtures[matchday]?.map((game) => {
                const isLocked = isGameLockedByDate(game.time);
                const isCurrentJoker = jokers[matchday] === game.id;
                return (
                    <div key={game.id} className={`p-4 rounded-xl border ${isCurrentJoker ? 'border-yellow-500 bg-amber-950/20' : 'border-gray-800 bg-gray-900'}`}>
                        <div className="flex justify-between text-xs text-gray-400 mb-2">
                           <span>{game.time}</span>
                           <button onClick={() => toggleJoker(game.id, isLocked)} className="font-bold">🃏 ג'וקר</button>
                        </div>
                        ...
                        <div className="flex justify-center gap-2">
                            {['1', 'X', '2'].map(o => <button key={o} disabled={isLocked} onClick={() => handlePredict(game.id, o)} className={`p-2 rounded-lg w-full ${predictions[`${matchday}-${game.id}`]?.winner === o ? 'bg-yellow-500 text-black' : 'bg-gray-800'}`}>{o}</button>)}
                        </div>
                    </div>
                )
            })}
             <button onClick={() => alert('נשמר!')} className="w-full bg-yellow-500 text-black font-black py-3 rounded-xl">שמור ניחושים</button>
          </>
        )}
        
        {currentTab === 'tournament' && (
           <div className="space-y-4">
               <input className="w-full bg-gray-800 p-3 rounded-lg" placeholder="אלופה..." value={tournamentPredictions.champion} onChange={e => setTournamentPredictions({...tournamentPredictions, champion: e.target.value})} />
               <input className="w-full bg-gray-800 p-3 rounded-lg" placeholder="מלך שערים..." value={tournamentPredictions.topScorer} onChange={e => setTournamentPredictions({...tournamentPredictions, topScorer: e.target.value})} />
               <button onClick={() => alert('נשמר!')} className="w-full bg-yellow-500 text-black font-black py-3 rounded-xl">שמור שינויים</button>
           </div>
        )}
      </div>

      <footer className="max-w-md mx-auto mt-6 text-center">
        <button onClick={loginAsAdmin} className="text-gray-500 text-xs font-bold">{isAdminMode ? 'צא ממנהל' : 'כניסת מנהל'}</button>
      </footer>
    </div>
  );
}
